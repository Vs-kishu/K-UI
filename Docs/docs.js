...
try {
  const mainSidebar = document.querySelector("aside.ui-drawer-modal");
  ...
  hamburgers.forEach((btn) =>
    btn.addEventListener("click", (e) => {
      ...
      if (toggleTarget.className.includes("ui-drawer-modal")) {
  ...
  document.body.addEventListener("click", (e) => {
    if (mainSidebar.className.includes("ui-drawer-modal")) {
...
try {
  ...
  const tabToggleBtns = document.querySelectorAll(
    ".main-demo-content .tabs-btn-list .ui-btn"
  );
  let currentActiveBlock = 1;

  let hideCurrentActiveBlocks = (blockNumber) => {
    document
      .getElementById(`preview-block-${blockNumber}`)
      .classList.add("hide");
  ...
  let showBlocks = (blockNumber) => {
    ...
    document
      .getElementById(`code-block-${blockNumber}`)
      .classList.remove("hide");
  };
  ...
  toggleVariantBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document
        .querySelector(".toggle-variant-btn.active")
        .classList.remove("active"); //remove active from current active btn
...
try {
  ...
  let currentActiveTabBtn = document.querySelectorAll(
    ".tabs-btn-list .ui-btn"
  )[0];
  const tabBtnWrapper = document.querySelector(".tabs-btn-list");
  const buttonBottomBorder = document.querySelector(
    ".tabs-btn-list .bottom-border"
  );
  let moveBorderBy = 0;

  tabBtnWrapper.addEventListener("click", (e) => {
    if (
      e.target.className.includes("ui-btn") &&
      e.target !== currentActiveTabBtn
    ) {
